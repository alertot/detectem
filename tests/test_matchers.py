...
class TestMatchers:
    ...
    @pytest.mark.parametrize(
        'matcher_class,entry,matcher,version', [
            ...
            (HeaderMatcher, res_server_header('foo-1.1'), ('Server', version_re), '1.1'),
...
class TestUrlMatcher:
    @pytest.mark.parametrize(
        'entry', [{'request': {'url': 'http://d.tld/foo-1.1'}}, {
            'request': {'url': ''},
            'response': {'url': 'http://d.tld/foo-1.1'},
        }]
    )
